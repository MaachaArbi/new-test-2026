...
final class AddBookingHotelRoomControllerTest extends WebTestCase
{
    public function test_add_hotel_room_returns_201_without_internal_id(): void
    {
        $client = static::createClient();
        $token = $this->authenticate($client);
        $booking = $this->seedBooking('hotel', 'HotelRoomOk');

        $client->request(
            'POST',
            '/api/v1/bookings/'.$booking['publicId'].'/hotel-rooms',
            server: [
                'CONTENT_TYPE' => 'application/json',
                'HTTP_ACCEPT' => 'application/json',
                'HTTP_AUTHORIZATION' => 'Bearer '.$token,
            ],
            content: json_encode(['roomType' => 'Chambre Double'], JSON_THROW_ON_ERROR),
        );
        ...
        self::assertSame('Chambre Double', $payload['roomType']);
        self::assertCount(1, $payload);
    }

    public function test_add_hotel_room_on_missing_booking_returns_404(): void
    {
        $missing = '00000000-0000-4000-8000-000000000044';
        $client = static::createClient();
        $token = $this->authenticate($client);

        $client->request(
            'POST',
            '/api/v1/bookings/'.$missing.'/hotel-rooms',
            server: [
                'CONTENT_TYPE' => 'application/json',
                'HTTP_ACCEPT' => 'application/json',
                'HTTP_AUTHORIZATION' => 'Bearer '.$token,
            ],
            content: json_encode(['roomType' => 'Double'], JSON_THROW_ON_ERROR),
        );
    ...
    public function test_add_hotel_room_on_flight_booking_returns_409(): void
    {
        $client = static::createClient();
        $token = $this->authenticate($client);
        $booking = $this->seedBooking('flight', 'HotelRoomMismatch');
        ...
        $client->request(
            ...
            content: json_encode(['roomType' => 'Should Fail'], JSON_THROW_ON_ERROR),
        );
        ...
        self::assertSame('accommodation', $payload['error']['context']['extension_code']);
        self::assertSame('flight', $payload['error']['context']['actual_service_type']);
    }

    public function test_malformed_room_type_returns_422(): void
    {
        $client = static::createClient();
        $token = $this->authenticate($client);
        $booking = $this->seedBooking('hotel', 'HotelRoom422');
        ...
        $client->request(
            ...
            content: json_encode(['roomType' => 42], JSON_THROW_ON_ERROR),
        );

        self::assertResponseStatusCodeSame(422);
    }

    public function test_add_hotel_room_without_jwt_returns_401(): void
    {
        $client = static::createClient();

        $client->request(
            'POST',
            '/api/v1/bookings/00000000-0000-4000-8000-000000000043/hotel-rooms',
            server: [
                'CONTENT_TYPE' => 'application/json',
                'HTTP_ACCEPT' => 'application/json',
            ],
    ...
    private function authenticate(KernelBrowser $client): string
    {
        ...
        $email = 'http.bk.hotelroom.'.$suffix.'@example.com';
        $password = 'Http-Bk-HotelRoom-'.$suffix;
        ...
        $account = PartyAccount::createPerson('Http Bk HotelRoom '.$suffix, Email::fromString($email));
    ...
    private function seedBooking(string $serviceTypeCode, string $label): array
    {
        ...
        $customer = PartyAccount::createOrganization(
            'Hr Cust '.$label.' '.$suffix,
            Email::fromString('hr.cust.'.$suffix.'@example.com'),
        );
        $office = PartyAccount::createOrganization(
            'Hr Off '.$label.' '.$suffix,
            Email::fromString('hr.off.'.$suffix.'@example.com'),
        );
        ...
        $folder = BookingFolder::create('HR-'.$suffix, (int) $customer->id(), (int) $office->id());
        $folders->save($folder);
        $unitOfWork->commit();
