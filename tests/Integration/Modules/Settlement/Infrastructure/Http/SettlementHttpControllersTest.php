...
namespace App\Tests\Integration\Modules\Settlement\Infrastructure\Http;
 ...
 */
final class SettlementHttpControllersTest extends WebTestCase
{
    ...
    public function test_transition_status_ok_and_unchanged_conflict(): void
    {
        ...
        $client->request(
            'PATCH',
            '/api/v1/settlements/instruments/'.$instrument['publicId'].'/status',
            server: $this->jsonHeaders($token),
        ...
        self::assertSame('settlement_instrument.status_unchanged', $err['error']['code']);
    }
    ...
    public function test_transition_missing_instrument_returns_404(): void
    {
        ...
        $client->request(
            'PATCH',
            '/api/v1/settlements/instruments/00000000-0000-4000-8000-000000000009/status',
            server: $this->jsonHeaders($token),
            content: json_encode(['status' => 'cancelled'], JSON_THROW_ON_ERROR),
        );
        ...
        /** @var array{error: array{code: string}} $err */
        $err = json_decode((string) $client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        self::assertSame('settlement_instrument.not_found', $err['error']['code']);
    }
    ...
    public function test_post_credit_201_and_rejects_non_active(): void
    {
        ...
        $client->request(
            'POST',
            '/api/v1/settlements/instruments/'.$instrument['publicId'].'/credit',
            server: $this->jsonHeaders($token),
        );
        self::assertResponseStatusCodeSame(201);
        ...
        $client->request(
            'PATCH',
            '/api/v1/settlements/instruments/'.$other['publicId'].'/status',
            server: $this->jsonHeaders($token),
            content: json_encode(['status' => 'returned'], JSON_THROW_ON_ERROR),
        );
        $client->request(
            'POST',
            '/api/v1/settlements/instruments/'.$other['publicId'].'/credit',
            server: $this->jsonHeaders($token),
        );
        self::assertResponseStatusCodeSame(409);
        /** @var array{error: array{code: string}} $err */
        $err = json_decode((string) $client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        self::assertSame('settlement_instrument.not_active', $err['error']['code']);
    }
    ...
    public function test_matching_create_unmatch_and_exceeds_credit(): void
    {
        ...
        /** @var array{error: array{code: string}} $err */
        $err = json_decode((string) $client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        self::assertSame('settlement_matching.exceeds_credit', $err['error']['code']);
        ...
        $client->request(
            'DELETE',
            '/api/v1/settlements/matchings/'.$matching['publicId'],
            server: $this->jsonHeaders($token),
        );
        self::assertResponseStatusCodeSame(200);
    ...
    public function test_matching_not_found_and_malformed(): void
    {
        ...
        $client->request(
            'DELETE',
            '/api/v1/settlements/matchings/00000000-0000-4000-8000-000000000009',
            server: $this->jsonHeaders($token),
        );
        self::assertResponseStatusCodeSame(404);
    ...
    public function test_balance_get_and_party_404(): void
    {
        ...
        $client->request(
            'GET',
            '/api/v1/party-accounts/'.$book['partyPublicId'].'/settlements/balance',
            server: $this->jsonHeaders($token),
        );
        self::assertResponseStatusCodeSame(200);
        ...
        $client->request(
            'GET',
            '/api/v1/party-accounts/00000000-0000-4000-8000-000000000009/settlements/balance',
            server: $this->jsonHeaders($token),
        );
        self::assertResponseStatusCodeSame(404);
    }
    ...
    public function test_http_end_to_end_instrument_credit_matching_balance(): void
    {
        ...
        /** @var SettlementLedgerEntryRepositoryInterface $ledger */
    ...
    public function test_all_settlements_http_responses_never_expose_id_or_last_entry_id(): void
    {
        $client = static::createClient();
        $token = $this->authenticate($client);
        ...
        $client->request(
            'POST',
            '/api/v1/settlements/instruments/'.$instrument2['publicId'].'/credit',
            server: $this->jsonHeaders($token),
        );
        self::assertResponseStatusCodeSame(201);
    ...
    private function seedPartyAndPaymentMethod(): array
    {
        ...
        /** @var PartyAccountRepositoryInterface $accounts */
        $accounts = $container->get(PartyAccountRepositoryInterface::class);
        /** @var SettlementPaymentMethodRepositoryInterface $methods */
        $methods = $container->get(SettlementPaymentMethodRepositoryInterface::class);
    ...
    private function seedDebitAndCreditViaApp(int $debit, int $credit): array
    {
        ...
        /** @var PartyAccountRepositoryInterface $accounts */
        $accounts = $container->get(PartyAccountRepositoryInterface::class);
        /** @var SettlementEntryTypeRepositoryInterface $types */
        $types = $container->get(SettlementEntryTypeRepositoryInterface::class);
        /** @var SettlementLedgerEntryRepositoryInterface $ledger */
        $ledger = $container->get(SettlementLedgerEntryRepositoryInterface::class);
