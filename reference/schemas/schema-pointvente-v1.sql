-- ============================================================
-- Module         : Point de vente (pointvente_)
...
--                   indépendamment)
-- ============================================================
-- Tables legacy remplacées : ost_pointvente (structure jamais reprise
-- telle quelle -- conception neuve à partir du besoin réel exposé,
-- voir modele-conceptuel-pointvente.md)
...
COMMENT ON TABLE pointvente IS 'Site physique secondaire rattaché à un party_account_office. Volontairement PAS un party_account : pas d''identité fiscale, aucun rôle transactionnel/financier -- simple dimension de référence pour affichage client et reporting. Ne pas y ajouter de logique de règlement, de plafond ou de fonction/permission -- ça romprait la frontière party_account = acteur économique.';
COMMENT ON COLUMN pointvente.office_account_id IS 'Doit référencer un party_account portant party_account_office -- règle applicative, pas une contrainte SQL (même logique que party_account_office_relation.office_account_id, cf. schema-party-account-v1.sql).';
COMMENT ON COLUMN pointvente.is_active IS 'Désactivation simple (site fermé). Jamais de suppression physique : l''historique de réservation (booking.pointvente_id / pointvente_paiement_id, à venir côté Booking) doit rester résolvable indéfiniment.';

CREATE INDEX idx_pointvente_office ON pointvente(office_account_id) WHERE is_active = true;
CREATE UNIQUE INDEX uq_pointvente_public_id ON pointvente(public_id);

CREATE TRIGGER trg_pointvente_updated_at BEFORE UPDATE ON pointvente
...
--    dès qu'un export sera fourni (legacy ost_pointvente).
-- ============================================================
-- FIN
-- ============================================================
