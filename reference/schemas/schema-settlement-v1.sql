-- ============================================================================
-- schema-settlement-v1.sql — Module Règlements Client/Fournisseur
...
CREATE TABLE settlement_payment_method (
...
COMMENT ON TABLE settlement_payment_method IS
...
CREATE UNIQUE INDEX uq_settlement_payment_method_public_id ON settlement_payment_method(public_id);
...
CREATE TABLE settlement_entry_type (
    id            BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    public_id     UUID NOT NULL DEFAULT gen_random_uuid(),
...
-- ============================================================
CREATE TABLE settlement_instrument (
    id                 BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    public_id          UUID NOT NULL DEFAULT gen_random_uuid(),
    ...
    payment_method_id  BIGINT NOT NULL REFERENCES settlement_payment_method(id),
    ...
    --   amount_minor - SUM(matched_amount_minor) sur settlement_matching actif.
...
COMMENT ON TABLE settlement_instrument IS
'Pièce de règlement (chèque, virement, espèces, CB, bon de commande...).
 Un instrument produit des crédits dans le grand livre via settlement_matching.
...
CREATE UNIQUE INDEX uq_settlement_instrument_public_id ON settlement_instrument(public_id);
CREATE INDEX idx_settlement_instrument_party ON settlement_instrument(party_account_id, currency_code);
CREATE INDEX idx_settlement_instrument_status ON settlement_instrument(status_code)
    WHERE status_code <> 'active';

-- ============================================================
-- TRANSFERT DE SOLDE
-- Déclaré avant settlement_ledger_entry car celle-ci le référence.
-- ============================================================
CREATE TABLE settlement_transfer (
    ...
    -- L''annulation passe aussi par settlement_post_transfer_reversal()
    -- pour créer les deux jambes inverses.
    reverses_transfer_id BIGINT REFERENCES settlement_transfer(id),
...
COMMENT ON TABLE settlement_transfer IS
'Transfert de solde entre deux livres quelconques (même devise).
 ...
 Toujours créé via settlement_post_transfer() — jamais d''INSERT direct.
 La FK transfer_id sur settlement_ledger_entry empêche toute jambe orpheline.';

CREATE UNIQUE INDEX uq_settlement_transfer_public_id ON settlement_transfer(public_id);
CREATE INDEX idx_settlement_transfer_source ON settlement_transfer(source_account_id, currency_code);
CREATE INDEX idx_settlement_transfer_target ON settlement_transfer(target_account_id, currency_code);
...
CREATE TABLE settlement_ledger_entry (
    ...
    transfer_id        BIGINT REFERENCES settlement_transfer(id),

    -- Contre-passation : pointe l''écriture annulée.
    -- Modification de montant : reverse de l''ancienne obligation + nouvelle.
    reverses_entry_id  BIGINT REFERENCES settlement_ledger_entry(id),
...
COMMENT ON TABLE settlement_ledger_entry IS
'Grand livre append-only. Une écriture = un fait économique daté et immuable.
 ...
 elles existent (settlement_post_transfer ; à terme settlement_post_obligation,
 settlement_post_credit, settlement_post_reversal — voir NOTES D''IMPLÉMENTATION
 ...
 ignore le signe attendu (settlement_entry_type.normal_sign) ou la
...
CREATE INDEX idx_settlement_ledger_book ON settlement_ledger_entry
    (party_account_id, party_role, currency_code, effective_date, id);

CREATE UNIQUE INDEX uq_settlement_ledger_entry_public_id ON settlement_ledger_entry(public_id);
CREATE INDEX idx_settlement_ledger_booking   ON settlement_ledger_entry(booking_id)   WHERE booking_id   IS NOT NULL;
CREATE INDEX idx_settlement_ledger_instrument ON settlement_ledger_entry(instrument_id) WHERE instrument_id IS NOT NULL;
CREATE INDEX idx_settlement_ledger_transfer  ON settlement_ledger_entry(transfer_id)  WHERE transfer_id  IS NOT NULL;

-- IMMUABILITÉ : garantie structurelle, pas seulement par convention.
CREATE OR REPLACE FUNCTION settlement_ledger_block_mutation()
...
CREATE TRIGGER trg_settlement_ledger_no_mutation
    BEFORE UPDATE OR DELETE ON settlement_ledger_entry
    FOR EACH ROW EXECUTE FUNCTION settlement_ledger_block_mutation();
...
-- ============================================================
CREATE TABLE settlement_balance (
    party_account_id   BIGINT NOT NULL REFERENCES party_account(id),
    party_role         VARCHAR(20) NOT NULL,
    currency_code      VARCHAR(3) NOT NULL REFERENCES ref_currency(code),
    balance_minor      BIGINT NOT NULL DEFAULT 0,
    -- + tiers nous doit / − avance ou créditeur
    last_entry_id      BIGINT REFERENCES settlement_ledger_entry(id),
    entry_count        BIGINT NOT NULL DEFAULT 0,
    updated_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (party_account_id, party_role, currency_code)
);

COMMENT ON TABLE settlement_balance IS
'Snapshot de solde par (compte, rôle, devise), maintenu par trigger.
 ...
 Reconcilie toujours avec SUM(amount_minor) à froid sur settlement_ledger_entry.
...
CREATE OR REPLACE FUNCTION settlement_balance_apply()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER trg_settlement_balance_apply
    AFTER INSERT ON settlement_ledger_entry
    FOR EACH ROW EXECUTE FUNCTION settlement_balance_apply();
...
CREATE TABLE settlement_matching (
    ...
    debit_entry_id        BIGINT NOT NULL REFERENCES settlement_ledger_entry(id),
    credit_entry_id       BIGINT NOT NULL REFERENCES settlement_ledger_entry(id),
...
COMMENT ON TABLE settlement_matching IS
...
CREATE INDEX idx_settlement_matching_debit  ON settlement_matching(debit_entry_id)  WHERE unmatched_at IS NULL;
CREATE INDEX idx_settlement_matching_credit ON settlement_matching(credit_entry_id) WHERE unmatched_at IS NULL;
CREATE UNIQUE INDEX uq_settlement_matching_public_id ON settlement_matching(public_id);

-- ============================================================
-- FONCTIONS DE POSTING — seuls chemins autorisés en écriture.
-- L''application n''insère jamais directement dans settlement_ledger_entry.
-- ============================================================

-- Transfert atomique (+ ses deux jambes) — voir modele-conceptuel-settlement.md
CREATE OR REPLACE FUNCTION settlement_post_transfer(
    p_source_id    BIGINT,
...
COMMENT ON FUNCTION settlement_post_transfer IS
'Crée un transfert de solde + ses 2 jambes dans une seule transaction.
 ...
 Résultat : id du settlement_transfer créé.';

-- ============================================================
-- DONNÉES DE RÉFÉRENCE INITIALES
-- ============================================================

INSERT INTO settlement_payment_method (code, label, is_cash_like) VALUES
...
--
-- 2. Concurrence sur settlement_balance : le trigger AFTER INSERT
...
--    - settlement_post_obligation()  : projection d''une obligation depuis Booking
--    - settlement_post_credit()      : crédit issu d''une pièce
--    - settlement_post_reversal()    : contre-passation d''une écriture existante
--    Ces fonctions suivent le même patron que settlement_post_transfer().
...
--    acceptable (voir commentaire de settlement_ledger_entry).
--
-- 4. SolvencyCheckerInterface (stub Booking, toujours true) :
--    l''implémentation réelle lira settlement_balance.balance_minor
--    pour le compte concerné et le comparera au plafond du futur
--    module Pricing/Finance.
--
-- 5. pg_partman : non inclus ici. settlement_ledger_entry n''est pas
