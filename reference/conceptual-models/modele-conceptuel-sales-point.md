...
**Statut** : ✅ Conception validée sur PostgreSQL 16 réel. Aucune donnée réelle de points de vente disponible à ce jour (legacy `ost_sales_point` non fourni) — à reconfronter dès qu'un export sera transmis.
...
**Conséquence pour ce module** : aucune colonne supplémentaire n'est nécessaire sur `sales_point` pour ce futur rapport — un simple `GROUP BY sales_point.id` sur les réservations suffira, une fois la politique de prime tranchée et le module 7 en place.

## Structure

Table unique `sales_point` (pas de séparation "vente"/"paiement" au niveau structurel — voir ci-dessous).
...
ALTER TABLE booking ADD COLUMN sales_point_id BIGINT REFERENCES sales_point(id);
ALTER TABLE booking ADD COLUMN sales_point_payment_id BIGINT REFERENCES sales_point(id);
```
...
- Index partiel `idx_sales_point_office` (`WHERE is_active = true`) vérifié via `pg_indexes`.

## Points restés ouverts

- Aucune donnée réelle de points de vente disponible à ce jour — structure à reconfronter dès qu'un export legacy (`ost_sales_point`) sera fourni.
- Ajustement FK côté Booking (`sales_point_id`, `sales_point_payment_id`) à faire dans une session dédiée.
